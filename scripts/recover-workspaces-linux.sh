#!/usr/bin/env bash
# recover-workspaces (Linux) — standalone, signed-release version.
# Extracted from packages/nexforce-studio-dashboard-api/src/routes/recover-workspaces.ts's
# UNIX_SH_BODY (Linux branch) for distribution as a release asset.
...
user_data="${XDG_CONFIG_HOME:-$HOME/.config}/com.differentai.openwork"
stop_pattern="openwork"
settle_seconds=1
start_openwork() {
  ( openwork >/dev/null 2>&1 & ) && return 0
  ( xdg-open openwork >/dev/null 2>&1 & ) && return 0
  return 1
}
...
# absolute path, so a local workspace at "/home/hugo/Foo" is stored under
# "/home/hugo/foo". This block logs, per local workspace:
...
try:
    ...
    # Build id → UI baseUrl / port map from openwork-workspaces.json.
    ui_by_id = {}
    ...
    for real, e in by_real.items():
        ...
        # Every workspace id we've seen for this real path — desktop AND server side.
        ...
        # Any workspacePorts key that starts-with the same directory prefix — surfaces
        # stale entries from earlier locations of the same workspace.
        near_keys = [k for k in wp_keys if k != norm_key and (norm_key in k or k in norm_key)]
        # Path status: exists as directory, is a symlink, resolves elsewhere.
        try:
        ...
        # Additional diagnostic: probe every candidate port to see who (if
        # anyone) is listening. openwork-server responds to /health so we can
        # tell openwork from a random other service that happens to hold the
        # port. Best-effort with 1s timeouts.
        import socket
        def _port_status(p):
            ...
            # Something is listening. Probe /health to see if it's openwork.
            try:
        ...
        # Probe every port that could plausibly be the openwork-server: the
        # wpHit (what the desktop's normalizeWorkspaceKey lookup returns),
        # the uiPort (what openwork-workspaces.json says), and every OTHER
        # port in workspacePorts (stale entries — might reveal that Hugo's
        # openwork-server is bound to one of them, just not the one that
        # matches the normalized key).
        probed = set()
        pairs = []
