...
#   2. Opens the user's default browser (xdg-open) at the Nexforce Work
#      sign-in page with ?installerPort=<port>&installerState=<nonce>.
...
result_file=$(mktemp -t nexforce-installer.XXXXXX 2>/dev/null || mktemp /tmp/nexforce-installer.XXXXXX)
...
if command -v xdg-open >/dev/null 2>&1; then
  xdg-open "$sign_in_url" >/dev/null 2>&1 || true
elif command -v sensible-browser >/dev/null 2>&1; then
  sensible-browser "$sign_in_url" >/dev/null 2>&1 || true
fi
